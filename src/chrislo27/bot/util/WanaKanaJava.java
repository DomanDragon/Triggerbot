...
public class WanaKanaJava {
	...
	public static final WanaKanaJava globalInstance = new WanaKanaJava(false);
	...
	private boolean _isCharConsonant(char chr, boolean includeY) {
		Pattern regexp = includeY ? Pattern.compile("[bcdfghjklmnpqrstvwxyz]")
	...
	public String _hiraganaToRomaji(String hira) {
		...
		while (cursor < len) {
			...
			while (chunkSize > 0) {
				...
				if (String.valueOf(chunk.charAt(0)).equals("っ") && chunkSize == 1
						&& cursor < (len - 1)) {
		...
		while (position < len) {
			...
			while (chunkSize > 0) {
				...
				if (String.valueOf(chunkLC.charAt(0)).equals("n")) {
					// Convert n' to ん
					if (mOptions.get(OPTION_IME_MODE) && chunk.length() == 2
							&& String.valueOf(chunkLC.charAt(1)).equals("'")) {
						chunkSize = 2;
						chunk = "nn";
						chunkLC = chunk.toLowerCase();
					}
					...
					if (chunk.length() > 2 && _isCharConsonant(chunkLC.charAt(1), false)
							&& _isCharVowel(chunkLC.charAt(2), true)) {
						chunkSize = 1;
				...
				if (chunk.length() > 1 && !String.valueOf(chunkLC.charAt(0)).equals("n")
						&& _isCharConsonant(chunkLC.charAt(0), true)
			...
			if (roma.length() > (position + 1) && mOptions.get(OPTION_IME_MODE)
					&& String.valueOf(chunkLC.charAt(0)).equals("n")) {
				if ((String.valueOf(roma.charAt(position + 1)).toLowerCase().equals("y")
						&& position == (len - 2)) || position == (len - 1)) {
